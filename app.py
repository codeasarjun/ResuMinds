...
def process_resume(resume_path, jd_content):
    with open(resume_path, 'rb') as f:
        ...
        for page_num in range(pdf_document.page_count):
            ...
            text += page.get_text()
    # Extracting candidate information manually
    candidate_name, candidate_email = extract_candidate_info(text)

    # Extracting education and skills information manually
    education_text, skills_text = extract_education_and_skills(text)

     

    
    # matching algorithm: count the occurrences of JD words in the resume
    ...
    # Adjusting the basic threshold 
    jd_length = len(jd_words)
    threshold = min(jd_length // 5, 10)

    # Assigning a  rating based on the match count
...
def upload():
    # Getting uploaded JD and resumes
    ...
    # Processing and analyze each resume
    results = []
